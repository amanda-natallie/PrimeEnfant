<section class="section">
    <div class="container">
        <section class="hero is-success is-fullheight" style="margin-top: -70px;">
            <div class="hero-body">
                <div class="container has-text-centered">
                    <div class="columns">
                        <div class="column is-4 is-offset-4">
                            <h3 class="title has-text-grey">Acessar Sistema</h3>
                            <p class="subtitle has-text-grey">Utilize E-mail e Senha.</p>
                            <div class="box">
                                
                                <figure class="avatar">
                                    <img src="<?= base_url(); ?>/assets/backend/img/sample.png" style="width: 128px">
                                </figure>
                                <?php if($this->session->flashdata('erro_login')){
                                    echo '<div class="notification is-danger">'.$this->session->flashdata('erro_login').'</div>';
                                }
                                 if ($this->session->flashdata('cadastrado_com_sucesso')) {
                                    echo '<div class="notification is-info">' . $this->session->flashdata('cadastrado_com_sucesso') . '</div>';
                                }
                                    echo validation_errors('<div class="notification is-danger">', '</div>');
                                    echo form_open('usuarios/login');
                                    ?>
                                    <div class="field">
                                        <div class="control">
                                            <input class="input is-large" type="email" placeholder="E-mail" autofocus="" name="user-usuario" >
                                        </div>
                                    </div>

                                    <div class="field">
                                        <div class="control">
                                            <input class="input is-large" type="password" placeholder="Senha" name="user-senha" >
                                        </div>
                                    </div>
                                <div class="field">
                                    <div class="control">
                                        <div id="recaptcha" class="g-recaptcha" data-sitekey="6LfBVqgUAAAAACTfktnQbCq3xmwsIIg4RNOAmwR5" style="width: 100%;"></div>
                                    </div>
                                </div>
                                    <button class="button is-block is-info is-large is-fullwidth">Login</button>
                                 <?= form_close(); ?>
                            </div>
                            <p class="has-text-grey">
                                <a href="<?= base_url() ?>cadastro">Cadastre-se</a> &nbsp;·&nbsp;
                                <a href="<?= base_url() ?>esqueci_senha">Esqueci minha senha</a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
</section>
<script async type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/bulma/0.7.5/css/bulma.css"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
<script async type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/bulma/0.7.5/css/bulma.css"></script>
<script src="https://www.google.com/recaptcha/api.js?render=6LfBVqgUAAAAACTfktnQbCq3xmwsIIg4RNOAmwR5"></script>
<script>
    grecaptcha.ready(function () {
        grecaptcha.execute('6LfBVqgUAAAAACTfktnQbCq3xmwsIIg4RNOAmwR5', {action: 'homepage'}).then(function (token) {
        });
    });
</script>
